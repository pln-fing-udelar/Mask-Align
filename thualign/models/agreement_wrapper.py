...
class AgreementWrapper(AlignmentModel):
    ...
    def forward(self, features):
        ...
        b_loss, b_log_output = self.b_model.cal_loss(b_logits, inverse_features["target"],
                                                     inverse_features["target_mask"])
        ...
        log_output = f"loss: {loss:.3f}, f_loss: {f_loss:.3f}, b_loss: {b_loss:.3f}, agree_loss: {alpha * agree_loss:.3f}"
        ...
        if getattr(self.params, "entropy_loss", False):
            ...
            entropy_loss = (entropy(f_weights_renorm, mask=bidirection_mask) + entropy(b_weights_renorm,
                                                                                       mask=bidirection_mask.transpose(
                                                                                           -1, -2)))

            loss = loss + beta * entropy_loss
            log_output = f"loss: {loss:.3f}, f_loss: {f_loss:.3f}, b_loss: {b_loss:.3f}, agree_loss: {alpha * agree_loss:.3f}, entropy_loss: {beta * entropy_loss:.3f}"
