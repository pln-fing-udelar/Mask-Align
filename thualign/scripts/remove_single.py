...
def main(args):
        ...
        with open(args.src) as sin, open(args.tgt) as tin, open(src_out, 'w') as sout, open(tgt_out, 'w') as tout:
            for s, t in zip(sin, tin):
                ...
                if len(s.strip().split()) <= 1 or len(t.strip().split()) <= 1:
                    print(f"Find one token sent pair({idx}): <{s}, {t}>")
                    cnt += 1
                else:
                    sout.write(s)
                    tout.write(t)
    else:
        with open(args.src) as sin, open(args.tgt) as tin, open(args.talp) as ain, open(src_out, 'w') as sout, open(
                tgt_out, 'w') as tout, open(args.talp + '.clean', 'w') as aout:
    ...
    print(f"Find one token sent pairs: {cnt}")
