...
def print_grad(x, name="x"):
    if type(x) == torch.Tensor:
        x.register_hook(lambda x: print(f"Norm - {name} {list(x.shape)}:{torch.norm(x)}\n {x}"))
...
def print_grad_norm(x, name="x", summary=True, verbose=True):
    if type(x) == torch.Tensor:
        ...
        if summary:
            ...
            scalar('grad_norm/' + name + '/normavg', torch.norm(x) / x.nelement(), get_global_step(),
...
def print_grad_max(x, name="x"):
    if type(x) == torch.Tensor:
        x.register_hook(lambda x: print(f"Grad max - {name} {list(x.shape)}:\n {torch.max(x).item()}"))
    elif type(x) == torch.nn.Module:
        pass  # TODO
